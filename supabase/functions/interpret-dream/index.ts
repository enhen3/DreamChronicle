...
请帮我解读这个梦境。要求：不要使用"玄学角度"、"心理学角度"等标题，将两个角度自然融合，并大量使用表情符号让内容更易读。`;
    ...
    // 使用更强大的匹配，确保移除所有变体
    console.log('原始输出:', interpretation.substring(0, 200));
    
    // 多次清理，确保彻底移除
    let cleaned = interpretation;
    
    // 移除所有形式的标题（包括独立成行、行首、行中等各种位置）
    const titlePatterns = [
      // 匹配开头标题
      /^\s*解读\s*：?\s*\n?/gmi,
      /^\s*梦境解析\s*：?\s*\n?/gmi,
      /^\s*梦境解读\s*：?\s*\n?/gmi,
      // 匹配独立的"玄学角度（周公解梦）"标题行（可能前后有空白）
      /^\s*玄学角度\s*(?:（周公解梦）|\(周公解梦\)|：)?\s*\n?/gmi,
      // 匹配文本中的"玄学角度（周公解梦）"
      ...
      /^\s*结语\s*(?:：)?\s*\n?/gmi,
      // 匹配数字序号开头的标题
      /^\s*[一二三四五六七八九十\d]+[\.、．]\s*(?:玄学|心理学|传统|弗洛伊德|总结|结语|鼓励|祝福)[^\n]*\n?/gmi,
      /[一二三四五六七八九十\d]+[\.、．]\s*(?:玄学|心理学|传统|弗洛伊德|总结|结语|鼓励|祝福)/gi,
    ];
    
    // 多次执行替换，确保彻底清除
    for (let i = 0; i < 3; i++) {
      titlePatterns.forEach(regex => {
        cleaned = cleaned.replace(regex, '');
      });
    }
    
    // 额外：使用简单的字符串替换作为最后保障
    // 直接替换所有可能的标题字符串（按长度从长到短排序，确保先匹配长字符串）
    const replacements = [
      ['解读：', ''],
      ['解读', ''],
      ['梦境解析：', ''],
    ...
    for (let i = 0; i < 3; i++) {
      replacements.forEach(([search, replace]) => {
        // 移除独立成行的标题（包括前后的空白和换行）
        cleaned = cleaned.split('\n').map(line => {
          const trimmed = line.trim();
          if (trimmed === search || trimmed === search.replace('：', ':') || trimmed === search.replace(':', '：')) {
            return '';
          }
          // 移除行中的标题
          return line.replace(new RegExp(search.replace(/[()（）[\]\\^$.*+?{}|]/g, '\\$&'), 'gi'), '');
        }).filter(line => line.trim() !== '').join('\n');
        
        // 在整个文本中移除标题（包括不在行首的情况）
        cleaned = cleaned.replace(new RegExp(search.replace(/[()（）[\]\\^$.*+?{}|]/g, '\\$&'), 'gi'), '');
      });
    }
    ...
    console.log('清理后输出:', cleaned.substring(0, 200));
    
    // 更新 interpretation 变量
